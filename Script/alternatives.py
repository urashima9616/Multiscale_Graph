...
#TODO: this algorithm should have the option to work like a Python generator, so that fitting is only done once per input original
...
def test1():
    ...
    assert simpletesters.sanity_test(ws)
    assert simpletesters.sanity_test(liu_chung)
    assert simpletesters.sanity_test(er)
    assert simpletesters.sanity_test(rand_noise1)
    assert simpletesters.sanity_test(rand_noise2)
    assert simpletesters.sanity_test(kron)
