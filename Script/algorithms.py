...
#determine the path structure for paths from source to target_node
    ...
    if search_method == 'bypass_path_only':  #find the length of just the bypass path = the path of second-shortest length
        ...
        target_found = False
        for d in xrange(1, new_edge_horizon):
            new_fringe = []
            for v in fringe:
                if v == target_node:
                    target_found = True
        ...
        particles = [[random.choice(good_nbs)] for i in xrange(100)] #start location of the particles
...
def do_coarsen(G, params):
    #CODES:Initialize 
    #G_coarse =nx.empty_graph(create_using=nx.DiGraph()); 
    ...
    seeds, home_nodes, aggregates = seed_finder_weight_alg(G, params)
    ...
    for seed in seeds:
        ...
        '''
        for nb in aggregates[seed]:
            for nbnb in G_undirected.neighbors(nb):
                if nbnb in aggregates[seed]:
                    continue
                elif G.has_edge(nb,nbnb)
                free_edges.add((nb,nbnb))
                elif G.has_edge(nbnb,nb)
                free_edges.add((nbnb,nb))
...
def edit_edges_sequential(G, edge_edit_rate, edge_growth_rate, tpl_data, params):
    ...
    for trial_num in xrange(max(20, 3*target_edges_to_add)): 
        ...
        if long_bridging and head_to_tail_distance>1:
            back_node = head
            for dummy in xrange(head_to_tail_distance-1):
...
def interpolate_edges(G, c_data, model_map, fine_model_map, params):    
    ...
    return c_data
def interpolate_edges_directed(G, c_data, model_map, fine_model_map, params):    
    aggregates    = c_data['aggregates']
    merged_edges  = c_data['merged_edges']
    deep_copying  = params.get('deep_copying', True)
    if not deep_copying: assert len(model_map) == 0
...
#the distinction between model_map and fine_model_map is this: the former are clones, while the latter are children of clone nodes
    ...
    authentic_nodes = aggregates.keys() # all the seeds
    assert authentic_nodes != []
    edited_nodes = [node for node in G if node not in aggregates]
    num_new_nodes = 0
    num_new_edges = 0
    #Get all the new nodes aside from the current corase nodes
    for node in edited_nodes:
        ...
        renamed_nodes = {}
        #change the names of trapped nodes in one aggregate associated with new node
        ...
        #change names of trapped edges in one aggregates
        my_trapped_edges = []
        for edge_hosted_by_source in sources_edges:
            ...
            num_new_edges += 1
        #new node becomes seed and cover all the original nodes
        aggregates[node]    = my_trapped_nodes
        trapped_edges[node] = my_trapped_edges
        #change the home names for trapped nodes
...
def generate_graph(original, params=None):
    ...
    if params.get('verbose', True):
        print 'replica is finished. nn: %d'%replica.number_of_nodes()
        print
...
def seed_finder_weight_alg_directed(G, params):
    '''
    coarsening inspired by Safro et al. 
        Graph minimum linear arrangement by multilevel
        weighted edge contractions
    ...
    seed_threshold_1 = params.get('seed_threshold_1', 2.0)
    seed_threshold_2 = params.get('seed_threshold_2', 0.4)
    G=nx.Graph(G); # directed to undirected
    G_adj = G.adj
    G_degree    = lambda u: G_adj[u].__len__()
    G_neighbors = lambda u: G_adj[u].keys() # 
    expected_wt_dict = {}
